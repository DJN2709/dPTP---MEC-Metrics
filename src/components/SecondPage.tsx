...
const SecondPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    // Fetch data from both old (Jan 2024 - May 2025) and new (June 2025+) music endpoints
    Promise.all([
      fetch('http://localhost:4000/api/mec-highlevel').then(res => res.json()),
      fetch('http://localhost:4000/api/music-mec-highlevel').then(res => res.json())
    ]).then(([oldData, newData]) => {
      // Combine data from both endpoints and deduplicate by month
      const combinedData = [...oldData, ...newData];
      
      // Remove duplicates by keeping the last occurrence of each month
      const deduplicatedData = combinedData.reduce((acc: any[], current: any) => {
        const existingIndex = acc.findIndex(item => item.month === current.month);
        if (existingIndex >= 0) {
          // Replace existing entry with the current one (prioritize newer data)
      ...
      setError('Failed to fetch Music MEC high-level data');
      setLoading(false);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    // Fetch data from both old (Jan 2024 - May 2025) and new (June 2025+) music endpoints
    Promise.all([
      fetch('http://localhost:4000/api/mec-steps-timeline').then(res => res.json()),
      fetch('http://localhost:4000/api/music-mec-steps-timeline').then(res => res.json())
    ]).then(([oldData, newData]) => {
      // Combine data from both endpoints and deduplicate by month
      const combinedData = [...oldData, ...newData];
      ...
      setStepsData(deduplicatedData);
      setStepsLoading(false);
    }).catch(() => {
      setStepsError('Failed to fetch Music MEC steps timeline data');
      setStepsLoading(false);
    });
  }, []);
